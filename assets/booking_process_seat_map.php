...
$query_theatre = mysqli_query($db,"SELECT * FROM tbl_theatres WHERE theatre_id='".$_SESSION['theatreID']."'");
$row_theatre = mysqli_fetch_array($query_theatre);
$theatreName=$row_theatre['theatre_name'];
$theatreCity=$row_theatre['city'];
...
<div class="modal_content_theatre_seat_map">
    <div class="modal-header">
        <h6 class="modal-title">THEATRE SEAT MAP</h6>
    ...
    <div class="modal-body">
        <div class="movie-data">
            ...
            <h4><?php echo $theatreName?>, <?php echo $theatreCity?></h4>
        </div>
        ...
        <div class="booking_category_showtime" align="center">
            <div class="ticket_count_display">
                ...
                <div class="ticket_count"><?php echo $_GET['fullTicketCount']?></div>
            </div>
    ...
    <div class="seat-map">
        <div class="container-fluid">
            <div class="table-responsive">
                ...
                <?php
                $sql = "SELECT * FROM tbl_theatre_seat_categories WHERE theatre_id='".$_SESSION['theatreID']."'";
                $result = $db->query($sql);
                ...
                <div class="seat_category">
                    <h3 class="seat_category_name"><?php echo implode(',',json_decode($row['category_name']))?></h3>
                    
                    <?php
                        $fullPrice = 0;
                        $kidsPrice = 0;
                        while($row_price=$result_price->fetch_assoc()){
                            $fullPrice = $row_price['ticket_rate'];
                        }
                    ?>
                    
                    <div class="category_price" full_price="<?php echo $fullPrice ?>">
                        (
                        <?php
                        if($fullPrice!="")
                        {
                            echo '<span>Rs.'.$fullPrice.'</span>';
                        }
                        ?>
                        )
                    </div>
                    ...
                    <table class="table seat_map_wrap" style="border-top: 5px solid #6b6b6b;">
                        <tbody>
                            ...
                            $seatMapArray = array();
                            $seatIDArray = array();
                            $sql_seatMap = "SELECT * FROM tbl_seat_maps WHERE seat_category_id = '".$row_price['ticket_category_id']."'";
                            ...
                            <tr class="seat_row">
                                <td>
                                    <div class="seat_row_label"><?php 
                                    echo "ok";
                                    echo $alpha[$row['num_of_rows'] - $r] 
                                    ?></div>
                                </td>
...
.modal_content_theatre_seat_map {
    background-color: #FFF;
}
.modal_content_theatre_seat_map .modal-header{
    border-radius: 0;
	border-bottom: 2px solid #3e3e3e;
	background-color: #000;
	min-height: 55px;
}
.modal_content_theatre_seat_map .modal-header h6 {
	color: #fff;
	display: inline-block;
	margin-top: 8px;
	user-select: none;
}
.modal_content_theatre_seat_map .close {
	font-size: 38px;
	color: #fff;
    opacity: 1;
    outline:none;
}
.modal_content_theatre_seat_map .modal-body {
	background-color: #131313;
	user-select: none;
}
.modal_content_theatre_seat_map .modal-body .movie-data {
	margin-top: 15px;
	margin-bottom: 25px;
	text-align: center;
}
.modal_content_theatre_seat_map .modal-body .movie-data h2{
	font-size: 28px;
	font-family: sans-serif;
	color: #fff;
	font-weight: 300;
}
.modal_content_theatre_seat_map .modal-body .movie-data h4{
	font-size: 14px;
	font-family: sans-serif;
	color: #fff;
	font-weight: 500;
...
.modal_content_theatre_seat_map .modal-body hr {
	border-top: 1px solid #232323;
	width: 33%;
}
.modal_content_theatre_seat_map .modal-body .booking_category_showtime {
	padding-top: 7px;
	padding-bottom: 7px;
	font-family: 'Trebuchet MS', 'Lucida Sans Unicode', 'Lucida Grande', 'Lucida Sans', Arial, sans-serif;
}
.modal_content_theatre_seat_map .modal-body .booking_category_showtime .ticket_count_display{
	display: inline;
}
.modal_content_theatre_seat_map .modal-body .booking_category_showtime .ticket_count_display .ticket_count_label {
	display: inline;
	color: #FFF;
}
.modal_content_theatre_seat_map .modal-body .booking_category_showtime .ticket_count_display .ticket_count{
	display: inline;
	margin-left: 10px;
	text-align: center;
...
.modal_content_theatre_seat_map .modal-body .booking_category_showtime .showtime_display{
	display: inline;
	margin-left: 40px;
}
.modal_content_theatre_seat_map .modal-body .booking_category_showtime .showtime_display .showtime_label {
	display: inline;
	color: #FFF;
}
.modal_content_theatre_seat_map .modal-body .booking_category_showtime .showtime_display .showtime{
	display: inline;
	margin-left: 10px;
	text-align: center;
	font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif !important;
...
.modal_content_theatre_seat_map .show-date-total-amount {
...
.modal_content_theatre_seat_map .show-date-total-amount .col-sm-9 {
		width: 75%;
}}
.modal_content_theatre_seat_map .show-date-total-amount h4{    
	margin: 20px 0;
	font-size: 24px;
	font-weight: 600;
}
.modal_content_theatre_seat_map .show-date-total-amount .date-left h4{    
	text-align: left;
	color: #282828;
}
@media (min-width: 768px) {
...
.modal_content_theatre_seat_map .show-date-total-amount .total-right h4 {    
	text-align: right;
	color: #ad0b0b;
}
.modal_content_theatre_seat_map .seat-map {
	text-align: center;
	margin-top: 60px;
	user-select: none;
}
.modal_content_theatre_seat_map .seat-map .container-fluid {
	padding-right: 15px;
	padding-left: 15px;
	margin-right: auto;
	margin-left: auto;
...
.modal_content_theatre_seat_map .seat-map .table-responsive {
	width: 100%;
	margin-bottom: 15px;
...
.modal_content_theatre_seat_map .seat-map .table-responsive .seat_category {
	text-align: center;
	margin: 0;
	width: 100%;
	font-family: 'Trebuchet MS', 'Lucida Sans Unicode', 'Lucida Grande', 'Lucida Sans', Arial, sans-serif;
}
.modal_content_theatre_seat_map .seat-map .table-responsive .seat_category .seat_category_name {
	color: #282828;
	font-size: 25px;
	margin-bottom: 0;
}
.modal_content_theatre_seat_map .seat-map .table-responsive .seat_category .category_price {
	font-weight: 400;
	color: #777;
}
.modal_content_theatre_seat_map .seat-map .table-responsive .seat_category .table {
	margin-bottom: 20px;
	border-spacing: 0;
	border-collapse: collapse;
	background-color: transparent;
}
.modal_content_theatre_seat_map .seat-map .table-responsive .seat_category table.seat_map_wrap {
	display: inline-block;
	text-align: center;
	padding: 2%;
	width: auto;
...
.modal_content_theatre_seat_map .seat-map .table.seat_map_wrap {
	width: 1040px !important;
	max-width: 1040px !important;
}}
.modal_content_theatre_seat_map .seat-map .table-responsive .seat_category .seat_map_wrap .screen_area span {
...
.modal_content_theatre_seat_map .seat-map .seat_map_wrap td {
	border-top: none !important;
	padding: 0 !important;
	line-height: 1.42857143;
}
.modal_content_theatre_seat_map .seat-map .table td {
	padding: 0;
	vertical-align: middle;
	border-top: none;
}
.modal_content_theatre_seat_map .seat-map .seat_map_wrap .seat_row_label {
...
.modal_content_theatre_seat_map .seat-map .seat_map_wrap .seat {
	float: left;
	margin: 4px 3px;
	width: 24px;
}
.modal_content_theatre_seat_map .seat-map .seat_map_wrap .seat a {
	display: inline-block;
	font-family: inherit;
	font-size: 10px;
	line-height: 26px;
...
.modal_content_theatre_seat_map .seat-map .seat_map_wrap .seat a:hover {
	color: #fff;
	background: green;
	text-decoration: none;
	transform: scale(1.6);
...
.modal_content_theatre_seat_map .seat-map .seat_map_wrap .seat .not_available_seat a {
	color: #fff;
	background: red;
	text-decoration: none;
	pointer-events: none;
...
.modal_content_theatre_seat_map .seat-map .seat_map_wrap .seat .selected_seat a{
	color: #fff;
	background: green;
	text-decoration: none;
	box-shadow: 0 0 0 1px green inset;
}
.modal_content_theatre_seat_map #bottom-next-wrapper {
	background-color: #000;
	padding: 15px 0;
	text-align: center;
}
.modal_content_theatre_seat_map #bottom-next-wrapper .next-btn {
	font-family: 'Trebuchet MS', 'Lucida Sans Unicode', 'Lucida Grande', 'Lucida Sans', Arial, sans-serif;
	font-weight: 700;
	background-color: transparent;
	color: #ad0b0b;
...
.modal_content_theatre_seat_map #bottom-next-wrapper .next-btn:hover {
	background-color: #d30f0f !important;
	color: #FFF;
	box-shadow: none !important;
}
.modal_content_theatre_seat_map #bottom-next-wrapper .next-btn:active {
	background-color:#9b0c0c !important;
	color: #FFF;
	box-shadow: none !important;
}
.modal_content_theatre_seat_map #bottom-next-wrapper .next-btn:focus {
	box-shadow: none !important;
}
@media (max-width: 575px){
	.modal_content_theatre_seat_map .modal-body .showtime_display{
...
<script>
    $(document).ready(function(){
        ...
        var theatre_name = "<?php echo $theatreName?>";
        var theatre_city = "<?php echo $theatreCity?>";
        ...
        var full_amount = 0;
        $('.amount_value').html(Number(full_amount).toFixed(2));
