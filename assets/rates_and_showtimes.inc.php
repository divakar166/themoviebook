...
$db = new mysqli("localhost","root","","theMovieBook");
...
if(isset($_POST["sel_date"]) && isset($_POST['movie_id']) && !empty($_POST["sel_date"]) && !empty($_POST['movie_id'])){
    ...
    $query_theatre = $db->query("SELECT A.*,A.screen, B.theatre_name, B.city FROM tbl_shows A, tbl_theatres B WHERE A.theatre_id = B.theatre_id AND A.movie_id='$movieID' AND A.starting_date<='$sel_Date' AND A.ending_date>='$sel_Date'");
    $rowCount_theatre = $query_theatre->num_rows;
    
    if($rowCount_theatre > 0){
        echo '<option value="0">Select theatre</option>';
        while($row = $query_theatre->fetch_assoc()){ 
            echo '<option value="'.$row['theatre_id'].'">'.$row['theatre_name'].' - '.$row['city'].' ( Screen '.$row['screen'].' )</option>';
        }
    }else{
